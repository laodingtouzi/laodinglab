...
(function() {
  ...
  function fmtPrice(n) {
    try { return parseFloat(n).toFixed(2); } catch(e) { return String(n); }
  }
  function fmtPct(n) {
    try { const v = parseFloat(n); return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; } catch(e) { return String(n); }
  }
  function fmtWan(n) {
    try { return (parseFloat(n) / 10000).toFixed(2); } catch(e) { return String(n); }
  }
  function retCls(n) {
    try { const v = parseFloat(n); return v > 0 ? 'up' : (v < 0 ? 'down' : ''); } catch(e) { return ''; }
  }
  ...
  function syncHoldingsTable(cloudHoldings) {
    ...
    tbody.querySelectorAll('tr').forEach(row => {
      ...
      // 对比并更新：股数(8)、市值(9)、盈亏%(10)、当前价(7)、状态(3)
      let changed = false;
